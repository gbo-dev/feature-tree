...
func completeWorktreeBranchesWithShortcuts(_ *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	if len(args) > 0 {
		return nil, cobra.ShellCompDirectiveNoFileComp
	}
...
func listLocalBranches(ctx *gitx.RepoContext) ([]string, error) {
	stdout, stderr, exitCode, err := gitx.RunGitCommon(ctx, "for-each-ref", "--format=%(refname:short)", "refs/heads")
	if err != nil {
		return nil, err
	}
	if exitCode != 0 {
